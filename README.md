...
- 전체 인프라를 Terraform으로 코드화 — `terraform apply` 한 번으로 전체 생성
...
- **CQRS**: 상품 서비스의 읽기(R2DBC 비동기)와 쓰기(JPA 트랜잭션) 경로를 분리하여 조회 부하가 쓰기에 영향을 주지 않도록 설계
- **BFF 병렬 호출**: WebFlux 기반 비동기 병렬 조회로 응답 시간 단축 (순차 350ms → 병렬 200ms)
